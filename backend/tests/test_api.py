...
All tests use the `isolated_app` fixture from conftest.py, which provides a
...
def test_open_case_no_evidence(isolated_app):
    """
    ...
    """
    # Step 1 — create case
    create_res = isolated_app.post("/api/cases", json={
    ...
    get_res = isolated_app.get(f"/api/cases/{case_id}")
...
def test_duplicate_case_number_returns_409(isolated_app):
    """
    ...
    first = isolated_app.post("/api/cases", json=payload)
    assert first.status_code == 200, first.text

    second = isolated_app.post("/api/cases", json=payload)
...
def test_case_lifecycle(isolated_app, dahua_img_path):
    """
    ...
    # 3. List Cases
    list_res = isolated_app.get("/api/cases")
    assert list_res.status_code == 200
    assert len(list_res.json()) >= 1

    # 4. Add Evidence
    ev_res = isolated_app.post(f"/api/cases/{case_id}/evidence", json={
        "path": dahua_img_path
    })

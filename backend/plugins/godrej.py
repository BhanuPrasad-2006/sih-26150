"""
godrej.py — Godrej Security Solutions DVR/NVR brand plugin.

DETECTION-ONLY STUB / UNVERIFIED
================================
This plugin does **not** parse Godrej disks. No reliable public
documentation of a Godrej DVR/NVR on-disk file-system or frame container
...
# Public forensic literature does not establish a Godrej DVR/NVR disk layout.
_UNVERIFIED_ASCII_MARKERS = (b"GODREJ", b"Godrej")
_DETECT_SCAN_BYTES = 32 * 1024 * 1024
_STRING_HIT_CONFIDENCE = 0.15  # honest low score; never a verified brand match


class GodrejPlugin(BrandPlugin):
    """Detection-only / unverified stub. Does not recover video."""

    name = "godrej"
    display_name = "Godrej (detection-only / unverified)"
        ...
        try:
            ...
            for marker in _UNVERIFIED_ASCII_MARKERS:
                if mm.find(marker, 0, limit) != -1:
                    log.info(
                        "Godrej detect(): unverified ASCII marker %r found; "
            ...
            log.debug("Godrej detect() error: %s", exc)
            return 0.0

    def version_hint(self) -> str:
        return (
            "Godrej — detection-only / unverified stub. "
    ...
    def list_recordings(self, img: "EvidenceImage") -> tuple[list[RawFrame], str]:
        note = (
            "Godrej index parsing is not implemented (detection-only / unverified stub)."
        )
        log.info(note)
        return [], note
    ...
    def carve(
        ...
        note = "Godrej format parsing not yet implemented — detection only."
        log.info(note)
        return [], note

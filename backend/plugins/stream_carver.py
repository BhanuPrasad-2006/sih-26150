"""
stream_carver.py — Standards-based video stream carving shared by every plugin
that has no verified vendor layout (Hikvision, and the generic fallback).

Recovers contiguous, structurally valid video streams from raw bytes using only
PUBLIC standards — no reverse-engineered vendor layout:

  * MPEG program streams (ISO/IEC 13818-1): pack header + PES packet chain,
...
them (the exporter/ffprobe step upgrades UNCERTAIN -> PARTIAL, never COMPLETE).
"""
...
def carve_standard_streams(
    ...
    try:
      for region in regions:
        ...
        note = (
            f"Stream carving stopped early: {exc}. "

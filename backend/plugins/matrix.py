"""
matrix.py — Matrix (Matrix Comsec / SATATYA) DVR/NVR brand plugin.

DETECTION-ONLY STUB / UNVERIFIED
================================
This plugin does **not** parse Matrix disks. No reliable public documentation
of a Matrix DVR/NVR on-disk file-system or frame container was found (unlike
Dahua DHAV or Hikvision's master-sector signature).

detect() may look for unverified product/vendor ASCII strings. A hit is
reported as low confidence only. Absence of those strings returns 0.0.
...
# "MATRIX" alone is too generic (false positives); longer vendor/product strings
# are used, still without a published on-disk layout to back them.
_UNVERIFIED_ASCII_MARKERS = (b"SATATYA", b"Matrix Comsec", b"MATRIX COMSEC")
_DETECT_SCAN_BYTES = 32 * 1024 * 1024
_STRING_HIT_CONFIDENCE = 0.15  # honest low score; never a verified brand match


class MatrixPlugin(BrandPlugin):
    """Detection-only / unverified stub. Does not recover video."""

    name = "matrix"
    display_name = "Matrix (detection-only / unverified)"
        ...
        try:
            ...
            for marker in _UNVERIFIED_ASCII_MARKERS:
                if mm.find(marker, 0, limit) != -1:
                    log.info(
                        "Matrix detect(): unverified ASCII marker %r found; "
            ...
            log.debug("Matrix detect() error: %s", exc)
            return 0.0

    def version_hint(self) -> str:
        return (
            "Matrix — detection-only / unverified stub. "
    ...
    def list_recordings(self, img: "EvidenceImage") -> tuple[list[RawFrame], str]:
        note = (
            "Matrix index parsing is not implemented (detection-only / unverified stub)."
        )
        log.info(note)
        return [], note
    ...
    def carve(
        ...
        note = "Matrix format parsing not yet implemented — detection only."
        log.info(note)
        return [], note

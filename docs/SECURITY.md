...
| API surface | Video playback (`GET /api/cases/{id}/video/{seg}`) needs a session and serves only the exported file recorded for that segment, and only from inside that case's exports folder; API docs and OpenAPI schema need a session; optional path allow-list `FORENSIC_EVIDENCE_ROOTS`; upload cap `SIH_MAX_UPLOAD_GB` (default 200) | `main.py`, `security.py` |

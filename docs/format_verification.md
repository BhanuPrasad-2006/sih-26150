...
| CP Plus disks are Dahua-compatible | Nothing found. Checked 2026-09-26: neither `gbatmobile/dhfs_extractor` nor `DmytroMoisiuk/DVR_Dahua` mentions CP Plus. An AI chat answer claiming CP Plus recorders are built on Dahua hardware and "tested against real production data" by dhfs_extractor is **not a source**: the repository's README says only that DHFS 4.1 is "commonly found in Chinese DVRs" and makes no CP Plus or production-data claim | **L0** | Hypothesis only. It is plausible (many CP Plus models are rebranded Dahua) but unproven for any specific model; the plugin keeps CP Plus at UNCERTAIN with an explicit note |
